...
import { ChevronLeft, CheckCircle, List, BookOpen } from 'lucide-react';

export default function AnalogDetail() {
  const { id } = useParams<{ id: string }>();
  const experiment = analogExperiments.find(exp => exp.id === id);

  if (!experiment) {
    return (
      <div className="py-20 text-center">
        <h1 className="text-2xl font-bold text-gray-900">实验未找到</h1>
        <Link to="/analog-circuits" className="text-primary-600 hover:underline mt-4 inline-block">
          返回列表
        </Link>
      </div>
    );
